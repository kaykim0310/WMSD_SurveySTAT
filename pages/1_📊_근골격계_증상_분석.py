...
st.set_page_config(page_title="근골격계 증상 분석", layout="wide")
st.title("📊 근골격계 자가증상 분석")

# 세션 상태 초기화
if 'musculo_calculated' not in st.session_state:
    st.session_state.musculo_calculated = False
if 'musculo_df' not in st.session_state:
    st.session_state.musculo_df = None
...
if uploaded_file is not None:
    ...
    try:
        ...
        # 세션 상태에 저장
        st.session_state.musculo_df = df_calculated
        st.session_state.musculo_calculated = True
        st.session_state.musculo_results = {}
        ...
        # 세션 상태에 결과 저장
        st.session_state.musculo_results = all_results_for_excel
